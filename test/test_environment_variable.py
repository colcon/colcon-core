...
def _test_environ_dict_win32():
    ...
    # Test move_to_end
    env5 = EnvironDict({'A': '1', 'B': '2'})
    env5.move_to_end('a')
    assert list(env5) == ['B', 'A']
    with pytest.raises(KeyError):
        env5.move_to_end('C')
...
def _test_environ_dict_posix():
    ...
    # Test move_to_end is case-sensitive on POSIX
    env2 = EnvironDict({'Foo': '1', 'Bar': '2'})
    env2.move_to_end('Foo')
    assert list(env2) == ['Bar', 'Foo']
    with pytest.raises(KeyError):
        env2.move_to_end('foo')
